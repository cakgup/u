(function () {
    ...
    root.innerHTML = `
      <main class="admin-page">
        <section class="panel login-card">
          ...
          <h1 class="login-title gradient-text">Admin Link Microsite</h1>
          <p class="login-desc">Masukkan token Google Apps Script untuk menambah, mengubah, dan menghapus tombol link.</p>
        ...
        <div class="admin-shell">
          <header class="admin-topbar">
            <div class="admin-brand">
              ...
              <div>
                <h1 class="admin-title gradient-text">Admin Link Microsite</h1>
                <p class="admin-subtitle">Cukup kelola tombol: rekam, ubah, dan hapus link.</p>
              </div>
            </div>
          ...
          <section class="admin-grid simple-admin-grid">
            <div class="panel">
              <div class="panel-header">
                <div>
                  <h2 class="panel-title">Form Link</h2>
                  <p class="panel-desc">Profil dan visual microsite bersifat statis di source code. Backend GAS hanya menyimpan daftar link.</p>
                </div>
              </div>
              ...
              <div class="panel">
                <div class="panel-header">
                  <div>
                    ...
                    <p class="panel-desc">Simulasi tampilan 1080 × 1920.</p>
                  </div>
                </div>
                <div class="panel-body">
      ...
      <form id="linkForm" class="form-section">
        ...
        <div class="form-grid">
          ...
          <div class="field-full">
            <label class="label" for="linkSubtitleInput">Subjudul</label>
          ...
          <div>
            ...
            <input id="linkColorInput" class="input" type="color" value="#073b31">
          </div>
          <div>
  ...
  function renderLinkList() {
    ...
    return `
      <section class="form-section">
        <h3 class="form-section-title">Daftar Link</h3>
  ...
  function renderLinkEditorCard(link) {
    const active = link.is_active === false || String(link.is_active).toLowerCase() === "false" ? "Nonaktif" : "Aktif";
    return `
      <article class="link-editor-card">
        <div class="link-card-head">
          <div>
            ...
            <p class="link-card-url">${h(link.subtitle || "")} ${link.subtitle ? "·" : ""} ${h(link.url || "")}</p>
          </div>
          <span class="pill-button">#${h(link.sort_order || "-")} · ${h(active)}</span>
        </div>
        <div class="button-row">
  ...
  async function renderAdminPage() {
    ...
    const root = $("#app");
    root.innerHTML = `<main class="loading-screen"><div><div class="spinner"></div><p class="login-desc">Memuat daftar link...</p></div></main>`;
    try {
  ...
  function buildFormLink() {
    return {
      ...
      button_color: $("#linkColorInput")?.value || "#073b31",
      text_color: "#FFFFFF",
  ...
  function fillLinkForm(id) {
    ...
    $("#linkColorInput").value = normalizeColor(link.button_color || "#073b31");
  ...
  async function deleteLink(id) {
    if (!id || !confirm("Hapus link ini dari Google Sheets?")) return;
    try {
  ...
  function resetLinkForm(clearMessage = true) {
    ...
    $("#linkIdInput").value = "";
    $("#linkForm")?.reset();
    $("#linkColorInput").value = "#073b31";
    $("#linkSortInput").value = String((state.links || []).length + 1);
    $("#linkActiveInput").checked = true;
    if (clearMessage) $("#linkMessage").className = "message hidden";
    updatePreview();
  }

  function normalizeColor(value) {
    const text = String(value || "").trim();
    return /^#[0-9a-f]{6}$/i.test(text) ? text : "#073b31";
  }
